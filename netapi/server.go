...
// Server exposes a core.Space over HTTP.
type Server struct {
	space            *core.Space
	mux              *http.ServeMux
	waitSem          chan struct{}
	mcp              *mcp.Server
	mcpBearerToken   string
	mcpAllowedOrigin map[string]struct{}
}

// NewServer returns an HTTP handler for the given space. If maxWaiters is
// positive, it limits concurrent blocking clients.
func NewServer(space *core.Space, maxWaiters int) *Server {
	return NewServerWithOptions(space, maxWaiters, Options{})
}

func NewServerWithOptions(space *core.Space, maxWaiters int, opts Options) *Server {
	srv := &Server{
		space:          space,
		mcp:            mcp.NewServer(space, acedocs.Commit),
		mcpBearerToken: opts.MCPBearerToken,
	}
	if len(opts.MCPAllowedOrigins) > 0 {
		srv.mcpAllowedOrigin = make(map[string]struct{}, len(opts.MCPAllowedOrigins))
		for _, origin := range opts.MCPAllowedOrigins {
			origin = strings.TrimSpace(origin)
			if origin != "" {
				srv.mcpAllowedOrigin[origin] = struct{}{}
			}
		}
	}
	if maxWaiters > 0 {
...
func (srv *Server) handleMCP(w http.ResponseWriter, r *http.Request) {
	if srv.mcpBearerToken != "" {
		const prefix = "Bearer "
		auth := r.Header.Get("Authorization")
		if !strings.HasPrefix(auth, prefix) ||
			subtle.ConstantTimeCompare([]byte(strings.TrimPrefix(auth, prefix)), []byte(srv.mcpBearerToken)) != 1 {
			w.Header().Set("WWW-Authenticate", `Bearer realm="ace-mcp"`)
			writeError(w, http.StatusUnauthorized, "missing or invalid bearer token")
			return
		}
	}

	if origin := r.Header.Get("Origin"); origin != "" && len(srv.mcpAllowedOrigin) > 0 {
		if _, ok := srv.mcpAllowedOrigin[origin]; !ok {
			writeError(w, http.StatusForbidden, "origin not allowed")
			return
		}
	}

	srv.mcp.ServeHTTP(w, r)
}
...
func (srv *Server) handleOut(w http.ResponseWriter, r *http.Request) {
	...
	if req.TTL != "" {
		var err error
		ttl, err = core.ParseISO8601Duration(req.TTL)
		if err != nil {
...
func (srv *Server) handleMatchTest(w http.ResponseWriter, r *http.Request) {
	...
	ok, err := core.MatchWithConfigContext(r.Context(), req.Object, req.Pattern, srv.space.Config())
	if err != nil {
...
func (srv *Server) resolveCallerID(r *http.Request) (string, error) {
	...
	if clientKey != "" {
		...
		if ident == nil {
			return "", core.ValidationErr(fmt.Errorf("invalid client key"))
		}
		return ident.ID, nil
	}
	...
	if aceID != "" {
		if !srv.space.Config().InsecureIDs {
			return "", core.ValidationErr(fmt.Errorf(
...
func handleDocFile(w http.ResponseWriter, r *http.Request) {
	...
	data, err := acedocs.Docs.ReadFile(name)
	if err != nil {
