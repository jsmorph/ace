...
func httpMatch(client *http.Client, baseURL string, op string, pattern string, callerID string, wait int, since string) (*core.Result, int, error) {
...
func httpRd(client *http.Client, baseURL string, pattern string, callerID string) (*core.Result, error) {
	r, _, err := httpMatch(client, baseURL, "rd", pattern, callerID, 0, "")
	return r, err
}

func httpRdSince(client *http.Client, baseURL string, pattern string, callerID string, since string) (*core.Result, error) {
	r, _, err := httpMatch(client, baseURL, "rd", pattern, callerID, 0, since)
	return r, err
}

func httpIn(client *http.Client, baseURL string, pattern string, callerID string) (*core.Result, error) {
	r, _, err := httpMatch(client, baseURL, "in", pattern, callerID, 0, "")
	return r, err
}

func httpStats(client *http.Client, baseURL string) (*core.Stats, error) {
...
func runScenarioWithConfig(s scenario, cfgFn func(*core.Config)) bool {
...
func httpInWithDelete(client *http.Client, baseURL string, pattern string, callerID string) (*core.Result, error) {
	r, _, err := httpMatch(client, baseURL, "in", pattern, callerID, 0, "")
	return r, err
}
...
func scenarioConcurrent(writers, readers, requests int, wait int, maxWaiters int, blocking string) error {
	...
	srv := netapi.NewServer(space, maxWaiters)
	ts := httptest.NewServer(srv)
	defer ts.Close()
...
func cmdTest(args []string) {
	...
	deleteCfg := func(cfg *core.Config) {
		cfg.Deletes = true
