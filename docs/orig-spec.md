...
- [ ]  Modify `ace out` to read from `stdin` if `--object` isn't
       provided.  Each line of input is its own object.
	   
- [ ]  Modify `ace in` and `ace rd` to read the pattern from `stdin`
       if `--pattern` isn't given or is `-`.
	  
- [ ]  Add a subcommand `ace expire` that runs the expiration process
       once.
...
- [ ] Explicit deletes: Like SQS, an `in` should be followed by a
	  ...
	  strong random id ("deletion ID"), which gets mapped internally
	  to the `ID` of the emitted object and returned with it. If a
	  `del(DID)` is not received in that interval, the object
	  automatically reappears in the space.  When `del(DID)` *is*
	  received in that interval, the object is deleted permanently.

      To implement this feature, we add two columns to `objects`:
	  `delete_id` and `invisible_until` (timestamp).  These columns
	  are updated updated before the object is emitted.  On `del`, the
	  record is deleted.  If `del` doesn't arrive soon enough, the
	  values don't need to be updated since the original queries
	  should work properly if `invisible_until` is in the past.
	  
	  It should not be possible for two `delete_id`s to be emitted for
      the same object in an overlapping interval. This
      requirement/behavior is critical.
